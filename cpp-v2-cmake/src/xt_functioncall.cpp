...
t_AliveFunctionCall XT_FunctionCall::merge_continuous_buf(vector<string> &s_a_func_buf,
														  XTLog &xt_log)
{
  t_AliveFunctionCall aliveFunction;
  ...
  size_t len = s_a_func_buf.size();
  aliveFunction.call_mark     = s_a_func_buf[0];
  aliveFunction.sec_call_mark = s_a_func_buf[1];
  aliveFunction.ret_mark      = s_a_func_buf[len - 2];
  aliveFunction.sec_ret_mark  = s_a_func_buf[len - 1];

  vector<string>::iterator it = s_a_func_buf.begin() + 2;
  for (; it != s_a_func_buf.end() - 2; ++it) {
    vRecord = XT_Util::split((*it).c_str(), '\t');
    string sIndex = vRecord.back();
    unsigned long index = stoul(sIndex, nullptr, 10);
    ...
    if (XT_Util::equal_mark(*it, flag::TCG_QEMU_LD)) {
      node = xt_log.getRecord(index).getSourceNode();
    } else if (XT_Util::equal_mark(*it, flag::TCG_QEMU_ST)) {
      node = xt_log.getRecord(index).getDestinationNode();
    }
  ...
  create_continuous_buf(vNode, aliveFunction);
...
void XT_FunctionCall::create_continuous_buf(std::vector<XTNode> &vNode,
                                            t_AliveFunctionCall &aliveFunction)
{
// XT_AliveBuffer aAliveBuffer;
	t_AliveContinueBuffer aliveBuffer;
